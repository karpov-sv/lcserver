...
// The action handler is also used on the target page, which carries a single
// terminate form but neither of the polling containers.
  ...
  function refreshState(){
    $.ajax({
      ...
      success: function(json){
        ...
        if (json.target_state)
          $('#target-state').text(json.target_state);
        if (json.chain_position)
          $('#chain-position').text(json.chain_position + '/' + json.chain_total);

        // Task finished, was revoked, or is no more linked to a running target
        if (json.ready || (json.target_id && !json.target_running)){
          state_done = true;
